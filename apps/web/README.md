...
#### 변경 검토 (ADR 0019 Phase 1 · Issue #7)

Explorer 상세 drawer 안의 **탭 하나**가 전부입니다. 새 라우트·좌측 nav 항목·화면을 만들지
않았고 위 화면 표와 라우트 레지스트리는 그대로입니다.

- 탭은 서버 카탈로그의 **`descriptor.dryRun`이 true일 때만** 나타납니다. 브라우저가
  판단하지 않으므로 배포가 검토를 끄면 탭 자체가 사라집니다.
- **상세 보기 탭은 그대로 읽기 전용입니다.** 편집기는 검토 탭에만 있고 서버가 정제한 상세
  YAML을 seed로 엽니다. 저장·적용·생성·삭제·force·change token 컨트롤이 없고, 검토는
  클러스터에 아무것도 남기지 않는 일시적 조회입니다. Deployment·Secret write 경로
  (ADR 0014)는 별개이며 이 슬라이스에서 바뀌지 않았습니다.
- **raw 매니페스트는 편집기의 일시 컴포넌트 상태와 POST 본문에만 존재합니다.** TanStack
  query/mutation 캐시를 쓰지 않습니다 — 캐시에 넣으면 요청이 끝난 뒤에도 남고 devtools·
  재시도 경로가 생깁니다. localStorage·sessionStorage·URL·query key·console·오류 문자열
  어디에도 쓰지 않고, 통로는 `src/api/client.ts` 하나입니다.
- **보내기 전에 막는 것은 빈 입력과 1MiB 초과뿐입니다.** 배포 상한(기본 256KiB)의 최종
  판정은 서버(413)입니다. 경계에서 UI가 서버보다 엄격하면 서버가 받아 줄 입력을 막습니다.
- **취소는 실제로 요청을 끊습니다.** 편집·재제출·대상 변경·drawer 닫힘에서 진행 중인 요청을
  abort합니다. 대상 신원은 `clusterId`·GVR·`namespace`·`name`·`uid`·`resourceVersion` 전부이며,
  응답이 그중 하나라도 다르면 **렌더하지 않습니다** — 늦게 도착한 응답이 다른 객체의 화면을
  덮지 않습니다.
- **오류는 알려진 code → 고정 한국어 문구로만** 표시합니다. 서버 `message`나 알 수 없는 오류
  값을 화면에 넣는 경로가 없고, 모르는 실패는 고정 fallback 한 문장입니다.
- 결과는 서버가 정제한 것만 그립니다 — 변경 목록·절단 여부·거절 사유·정제 경로.
  `unchanged`·`changed`·`rejected`를 같은 화면으로 접지 않습니다.

이 이슈에서 확인한 게이트는 `make check` · `make build` · `make test-web`(contract 검사 포함)입니다.
